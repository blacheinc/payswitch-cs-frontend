    ...
    <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-4">
                    <div className="flex items-center space-x-2 border p-3 rounded-md w-full cursor-pointer hover:bg-muted/50">
                      <input type="radio" name="format" id="csv" className="accent-primary" defaultChecked />
                      <Label htmlFor="csv" className="cursor-pointer flex-1">CSV (Raw Data)</Label>
                    </div>
                    <div className="flex items-center space-x-2 border p-3 rounded-md w-full cursor-pointer hover:bg-muted/50">
                      <input type="radio" name="format" id="pdf" className="accent-primary" />
                      <Label htmlFor="pdf" className="cursor-pointer flex-1">PDF Report</Label>
                    </div>
                  </div>
                </div>
                <div className="space-y-2">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{performanceMetrics.totalRequests.value.toLocaleString()}</div>
            <div className="text-xs text-muted-foreground mt-1 flex items-center">
              ...
              <span className={performanceMetrics.totalRequests.trend === 'up' ? 'text-green-500' : 'text-red-500'}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{performanceMetrics.approvalRate.value}%</div>
            <div className="text-xs text-muted-foreground mt-1 flex items-center">
              <ArrowUpRight className="h-3 w-3 text-green-500 mr-1" />
              <span className="text-green-500">{performanceMetrics.approvalRate.change}%</span>
              <span className="ml-1">vs previous period</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{performanceMetrics.avgScore.value}</div>
            <div className="text-xs text-muted-foreground mt-1 flex items-center">
              <ArrowDownRight className="h-3 w-3 text-red-500 mr-1" />
              <span className="text-red-500">{Math.abs(performanceMetrics.avgScore.change)}%</span>
              <span className="ml-1">vs previous period</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{performanceMetrics.avgProcessingTime.value}ms</div>
            <div className="text-xs text-muted-foreground mt-1 flex items-center">
              <ArrowDownRight className="h-3 w-3 text-green-500 mr-1" />
              <span className="text-green-500">{Math.abs(performanceMetrics.avgProcessingTime.change)}%</span>
            ...
            <CardContent>
              <div className="flex items-center justify-center h-[350px] text-muted-foreground bg-muted/20 rounded-lg border border-dashed">
                <div className="text-center">
                  ...
                  <p className="mt-2 font-medium">No performance anomalies detected</p>
                  <p className="text-sm">Model version lgbm_v2.3.1 operating within expected parameters</p>
                </div>
              </div>
            </CardContent>
          </Card>
