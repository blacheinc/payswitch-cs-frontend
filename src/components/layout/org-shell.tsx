...
export function OrgShell({ children }: OrgShellProps) {
  ...
  // Permissions come from the server-rendered layout, so the sidebar is
  // correct on first paint and stays put during a logout transition.
  const { can } = usePermissions();
  const visibleNavItems = navItems.filter(
    (item) => !item.perms || item.perms.some(can),
  );
